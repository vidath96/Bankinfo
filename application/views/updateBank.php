		...
		<div class="row ">
			...
			<div class="col-6 col-sm-10 ">
				...
				<?php foreach ($result as $res) : ?>
					<form action="<?= base_url('Bank/updateBank') ?>" method="POST">
						<div class="form-group">
							<label class="form-label">Account No</label><br>
							<input type="text" name="accountno" class="form-control" value="<?= $res->bank_account_no ?>" required>
							<input type="text" name="confirmaccno" class="form-control" value="<?= $res->bank_account_no ?>" hidden required>
							<input type="text" name="id" class="form-control" value="<?= $res->id ?>" hidden required>
						</div>

						<div class="form-group">
							<label class="label form-label">Bank Name</label><br>
							<input type="text" name="bankname" class="form-control" value="<?= $res->bank_name ?>" required>
						</div>
						<div class="form-group">
							<label class="label form-label">Branch </label><br>
							<input type="text" name="branch" class="form-control" value="<?= $res->branch ?>" required>
						</div>
						<div class="form-group">
							<label class="label form-label">Branch Code</label><br>
							<input type="number" name="branchcode" class="form-control" value="<?= $res->branch_code ?>" required>
						</div>
						<br>
					<?php endforeach; ?>
